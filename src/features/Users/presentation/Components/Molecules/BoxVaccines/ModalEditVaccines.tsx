...
function ModalEditVaccines({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (amount === currentAmount) {
      toast('La cantidad es la misma que la actual', {
        icon: 'ℹ️',
        duration: 3000,
        style: {
          borderRadius: '12px',
    ...
    try {
      ...
      toast.success(
        <div>
          <strong className="block mb-1">¡Cantidad actualizada!</strong>
          <p className="text-sm opacity-90">
            {amount > currentAmount 
              ? `Se agregaron ${amount - currentAmount} vacunas`
              : `Se redujeron ${currentAmount - amount} vacunas`
            }
          </p>
        </div>,
        {
